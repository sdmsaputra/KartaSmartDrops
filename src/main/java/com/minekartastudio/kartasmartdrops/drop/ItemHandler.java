package com.minekartastudio.kartasmartdrops.drop;

import com.minekartastudio.kartasmartdrops.KartaSmartDrops;
import com.minekartastudio.kartasmartdrops.config.ConfigData;
import com.minekartastudio.kartasmartdrops.config.DataContainer;
import com.minekartastudio.kartasmartdrops.config.StaticData;
import com.minekartastudio.kartasmartdrops.util.DivineTimer;
...
public class ItemHandler {
    ...
    public ItemHandler(@NotNull final KartaSmartDrops plugin,
        ...
        this.itemTickTimer = new DivineTimer(plugin, () -> {
            ...
            items.forEach(item -> {
                KartaSmartDrops.getMorePaperLib().scheduling().entitySpecificScheduler(item).run(item::remove, null);
            });
            items.clear();
        });
    }
    ...
    public void itemTick(@NotNull final Item item) {
        ...
        if (metaCountdowns.isEmpty()) {
            KartaSmartDrops.getMorePaperLib().scheduling().entitySpecificScheduler(item).run(() -> setupMetaTimer(item), null);
    ...
    public void updateTimedItem(@NotNull final Item item,
                                @NotNull final DataContainer container) {
        KartaSmartDrops.getMorePaperLib().scheduling().entitySpecificScheduler(item).run(() -> {
