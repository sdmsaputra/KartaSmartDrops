package com.minekartastudio.kartasmartdrops.util;
...
public class UpdateChecker {
    ...
    public void fetchLatestSupportedVersion(@NotNull final Consumer<@Nullable Version> handler, boolean sync) {
        new Thread(() -> {
            try {
                ...
                if (sync)
                    KartaSmartDrops.getMorePaperLib().scheduling().globalRegionalScheduler().run(() -> handler.accept(version));
                else
    ...
    public @Nullable Version fetchLatestSupportedVersion() throws IOException {
        ...
        connection.setRequestProperty("User-Agent", "KartaSmartDrops Update Checker");
